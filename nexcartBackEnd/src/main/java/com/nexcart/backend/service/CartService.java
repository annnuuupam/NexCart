...
import com.nexcart.backend.entity.ProductStatus;
...
public class CartService {
    ...
    public void addToCart(int userId, int productId, int quantity) {
        ...
        if (availableStock <= 0 || product.getProductStatus() == ProductStatus.OUT_OF_STOCK) {
        ...
        if (existingItem.isPresent()) {
            ...
            if (newQty > availableStock) {
                throw new InsufficientStockException("Only " + availableStock + " items left in stock");
            }
    ...
    public Map<String, Object> getCartItems(int userId) {
        ...
        for (CartItem cartItem : cartItems) {
            ...
            productDetails.put("stock", stock);
            productDetails.put("product_status", String.valueOf(product.getProductStatus()));
